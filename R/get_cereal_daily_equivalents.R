...
get_cereal_daily_equivalents <- function(data, cereal_names = c("cereal_type1", "cereal_type2")){
  ...
  for(v in 1:length(cereal_names)){
    ...
    assign(sprintf("n_%s", cereal_names[v]), length(data[data[[cereal_names[v]]] != "" & !is.na(data[[cereal_names[v]]]), cereal_names[v], drop = TRUE]))
    if(v == 1){
      data <- data %>% dplyr::left_join(calib_DSQ_cereal_ntile, by = dplyr::join_by(!!rlang::sym(cereal_names[v]) == Cereal_Name))
      if(get(sprintf("n_%s", cereal_names[v]), envir = environment()) != length(data[!is.na(data$food_code), "food_code", drop = TRUE])){
      ...
      if(get(sprintf("n_%s", cereal_names[v]), envir = environment()) != length(data[!is.na(data$food_code2), "food_code2", drop = TRUE])){
